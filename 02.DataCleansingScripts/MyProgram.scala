...
object MyProgram {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("file:///C:/data/data_Q4_2018/*.csv")
    ...
    failures.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").save("file:///C:/data/fail.csv")
   
    
     val success = spark.sql("SELECT * FROM peopleTemp where failure=0 and model='ST4000DM000'")
